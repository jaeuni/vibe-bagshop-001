...
### **[9단계] 3D 에셋 재질 노드 정밀 분석 및 파트 컬러 매핑 오류 정정**
- `src/components/Bag3DModel.tsx`에 동적 노드 로깅 코드를 내장하여, 브라우저 콘솔상에서 `aurelia_caviar_bag.glb` 에셋의 실제 머티리얼 및 메쉬 오브젝트 이름을 전수 조사.
- 실제 에셋 분석을 통해 가방 본체(Body), 가방끈(Strap), 그리고 금속 장식철물(Metal lock/Hardware)의 정확한 이름 매칭 배열을 확보.
- 오분류되던 분기 조건문을 전면 재수정하여, "가방 자체색 변경 시 가방 몸체 가죽만", "가방끈색 변경 시 끈/손잡이 가죽만" 명확하게 적용되게 하고, 잠금철물은 항상 고급스러운 메탈 골드 톤으로 우아하게 유지되도록 예외 필터 완비.
